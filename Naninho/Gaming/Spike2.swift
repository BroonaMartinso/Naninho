...
class Spike2 {
    ...
    var radius: CGFloat {ball.frame.width/2}
    ...
    func update(deltaTime: TimeInterval) {
        ...
        for spike in spikeArray {
            ...
            spike.position.y = ball.frame.midY+radius * cos(-spike.zRotation)
            spike.position.x = ball.frame.midX+radius * sin(-spike.zRotation)
        }
//        bate()
    }
...
//            new.physicsBody = nil
//            new.physicsBody = SKPhysicsBody(polygonFrom: path.cgPath)
...
//            new.physicsBody?.contactTestBitMask = 0b0001
//            new.physicsBody?.categoryBitMask = 0b0001
//            new.physicsBody?.collisionBitMask = 0b0001
//            new.physicsBody?.mass = 1
            
            let angulo = CGFloat(passo*Double(i))
            print(angulo)
            
            //Posiciona
            new.position.y = ball.frame.midY+radius*(cos(angulo * Double.pi / 180))
            new.position.x = ball.frame.midX+radius*(sin(angulo * Double.pi / 180))
